"""v73_recent400: XGB trained on RECENT 400 moons only. Local validator +0.008."""
...
def train(X_train, y_train, model_directory_path):
    ...
    # FILTER: only keep recent 400 moons
    last_moon = int(merged['moon'].max())
    recent = merged[merged['moon'] > last_moon - 400].reset_index(drop=True)
    print(f"[v73_400/train] Recent 400 moons: {len(recent)}/{len(merged)} rows, moons {recent['moon'].min()}..{recent['moon'].max()}")
