"""v73_recent200: XGB trained on RECENT 200 moons only. Local validator +0.008."""
...
def train(X_train, y_train, model_directory_path):
    ...
    # FILTER: only keep recent 200 moons
    last_moon = int(merged['moon'].max())
    recent = merged[merged['moon'] > last_moon - 200].reset_index(drop=True)
    print(f"[v73_200/train] Recent 200 moons: {len(recent)}/{len(merged)} rows, moons {recent['moon'].min()}..{recent['moon'].max()}")
