"""v65: 9-model ensemble — top 3 configs × 3 seeds (variance reduction max).

Configs: [400, 500, 600]/d6/lr=0.03, seeds=[42, 7, 2026].
Local pred: ~0.065 mean, std/sqrt(9)=0.013 → 1σ range [0.052, 0.078].
"""
...
def train(X_train, y_train, model_directory_path):
    ...
    for n_est in [400, 500, 600]:
        for seed in [42, 7, 2026]:
            print(f"[v65/train] n_est={n_est} seed={seed}")
            m = XGBRegressor(
