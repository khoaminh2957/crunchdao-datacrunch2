"""v73_recent300: XGB trained on RECENT 300 moons only. Local validator +0.008."""
...
def train(X_train, y_train, model_directory_path):
    ...
    # FILTER: only keep recent 300 moons
    last_moon = int(merged['moon'].max())
    recent = merged[merged['moon'] > last_moon - 300].reset_index(drop=True)
    print(f"[v73_300/train] Recent 300 moons: {len(recent)}/{len(merged)} rows, moons {recent['moon'].min()}..{recent['moon'].max()}")
