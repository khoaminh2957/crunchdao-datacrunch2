"""R19: 60-sample verification of lr=0.005 n=3000 (best R17 candidate).

R17 30-sample: +0.00587 t=1.83.
Goal: drive t-stat into significance (>=2.0 = p<0.05).
"""
...
for T in TRAIN_ENDS:
    ...
    for s in SEEDS:
        ...
        m_a = XGBRegressor(n_estimators=3000, max_depth=6, learning_rate=0.005,
                           subsample=0.8, colsample_bytree=0.5, tree_method='hist',
                           device=DEV, n_jobs=-1, random_state=s, verbosity=0)
        m_a.fit(Xt, yt)
        r_a = pearsonr(m_a.predict(Xte), y_te)[0]
        bl.append(r_b); alt.append(r_a)
        print(f"  T={T:<4} s={s:<5} bl={r_b:+.5f} lr005={r_a:+.5f} diff={r_a-r_b:+.5f}", flush=True)

bl=np.array(bl); alt=np.array(alt); diff=alt-bl
print(f"\n=== R19 lr=0.005 n=3000 verify (n={len(bl)}) ===")
print(f"  Baseline lr=0.03 n=500: mean={bl.mean():+.5f} std={bl.std():.5f}")
print(f"  Alt lr=0.005 n=3000:    mean={alt.mean():+.5f} std={alt.std():.5f}")
