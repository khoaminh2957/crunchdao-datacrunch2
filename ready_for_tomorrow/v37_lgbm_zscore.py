"""v37: LGBM (not XGB) on raw + per-moon z-score features.

v30 XGB(raw+z) = 0.0563. Test if LGBM with same feature engineering
matches or beats XGB on this richer feature set.
"""
import pandas as pd
import numpy as np
import pickle
...
def train(X_train, y_train, model_directory_path):
    ...
    print(f"[v37/train] LGBM(500/lr0.03/63 leaves) on {len(Xt)} rows × {len(feature_cols)} feats (raw+z)")
