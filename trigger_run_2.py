"""Trigger cloud run for submission #2."""
import json, urllib.request, urllib.parse, websocket, time, sys
sys.stdout.reconfigure(encoding='utf-8')
WS_KW = dict(origin="http://localhost", suppress_origin=False)

# Navigate to create-run for sub #2
url = "https://hub.crunchdao.com/competitions/datacrunch-2/models/12867/curly-crayfishwetminh-v2/runs/create?submissionNumber=2"
req = urllib.request.Request(f"http://localhost:9222/json/new?{urllib.parse.quote(url)}", method="PUT")
urllib.request.urlopen(req)
time.sleep(8)

tabs = json.loads(urllib.request.urlopen("http://localhost:9222/json").read())
tab = [t for t in tabs if "runs/create" in t.get("url","") and "submissionNumber=2" in t.get("url","") and t.get("type") == "page"][0]
...
# Enable network sniff
cdp(ws, "Network.enable", {}, 1)

# Same pattern that worked before
r = evaluate(ws, r"""
  (async () => {
    // 1. Click Yes I did
...
# Collect network events for 10s to capture POST response
events = []
ws.settimeout(0.5)
