"""Poll run #81537 status every 30s; print transitions."""
...
url = "https://hub.crunchdao.com/competitions/datacrunch-2/models/12867/curly-crayfishwetminh-v2/runs/81537"
...
tab = [t for t in tabs if "runs/81537" in t.get("url","") and t.get("type") == "page"][0]
...
for i in range(40):
    ...
    s = evaluate(ws, r"""
      (() => {
        ...
        const score = txt.match(/Score\s*\n+\s*([^\n]+)/);
        return {status: stat?.[1], duration: dur?.[1], score: score?.[1], tlen: txt.length};
      })()
    """, 100+i)
