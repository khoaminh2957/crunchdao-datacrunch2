"""v73_recent150: XGB trained on RECENT 150 moons only. Local validator +0.008."""
...
def train(X_train, y_train, model_directory_path):
    ...
    # FILTER: only keep recent 150 moons
    last_moon = int(merged['moon'].max())
    recent = merged[merged['moon'] > last_moon - 150].reset_index(drop=True)
    print(f"[v73_150/train] Recent 150 moons: {len(recent)}/{len(merged)} rows, moons {recent['moon'].min()}..{recent['moon'].max()}")
    yt = recent[target_col].values; Xt = recent[feature_cols]
    m = XGBRegressor(n_estimators=500, max_depth=6, learning_rate=0.03, subsample=0.8,
                     colsample_bytree=0.5, tree_method='hist', n_jobs=-1, random_state=42, verbosity=0)
    m.fit(Xt, yt)
