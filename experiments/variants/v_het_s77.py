"""v_het_s77: single-seed hetero ensemble, seed=77 (different from v68's 42). Lottery shot."""
...
def train(X_train, y_train, model_directory_path):
    ...
    m_xgb = XGBRegressor(n_estimators=500, max_depth=6, learning_rate=0.03, subsample=0.8,
                          colsample_bytree=0.5, tree_method='hist', n_jobs=-1, random_state=77, verbosity=0)
    m_xgb.fit(Xt, yt)
    m_lgbm = LGBMRegressor(n_estimators=500, learning_rate=0.03, num_leaves=31, min_data_in_leaf=200,
                            colsample_bytree=0.5, reg_lambda=2.0, verbose=-1, n_jobs=-1, random_state=77)
    m_lgbm.fit(Xt, yt)
    imp = SimpleImputer(strategy='median'); Xt_imp = imp.fit_transform(Xt)
    m_ridge = Ridge(alpha=10.0, random_state=77); m_ridge.fit(Xt_imp, yt)
